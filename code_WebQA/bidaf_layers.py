...
class BiAttentionLayer(nn.Module):
    def __init__(self, config, JX, M, JQ, input_feature_size, \
        ...
        self.JQ = JQ
        self.input_keep_prob = input_keep_prob
        # input_size: including h_aug, u_aug, hu_aug # 这个代码里面根本没写 hu_aug 嘛   这个能跑起来吗？ 确定不坑爹的？
...
class BiAttentionLayerMine(nn.Module):
    def __init__(self, dim_h, dim_u, input_keep_prob=1.0):
        super(BiAttentionLayerMine, self).__init__()
        self.input_keep_prob = input_keep_prob
        # input_size: including h_aug, u_aug, hu_aug
        assert dim_u == dim_h
        input_size = dim_h + dim_u + dim_u
        self.linear = nn.Linear(input_size, out_features=1)

    def forward(self, h, u, h_mask=None, u_mask=None):
        bs, seq_L_h, h_dim = h.size()
        bs, seq_L_u, u_dim = u.size()

        h_aug = h.unsqueeze(2).repeat(1, 1, seq_L_u, 1).view(bs * seq_L_h * seq_L_u, h_dim)
        u_aug = u.unsqueeze(1).repeat(1, seq_L_h, 1, 1).view(bs * seq_L_h * seq_L_u, h_dim)

        hu_aug = h_aug * u_aug
        liner_in = torch.cat((hu_aug, h_aug, u_aug), dim=1)
        u_logits = self.linear(liner_in).view(bs * seq_L_h, seq_L_u)

        u_logits_normized = F.softmax(u_logits).view(bs, seq_L_h, seq_L_u)

        u_aug = u_aug.view(bs, seq_L_h, seq_L_u, u_dim)
        u_a = u_logits_normized.unsqueeze(3).mul(u_aug).sum(2)
        h_a = torch.max(u_logits_normized, 2)[0].unsqueeze(2).mul(h).sum(1)  # bs * h_dim
        h_a = h_a.unsqueeze(1).repeat(1, seq_L_h, 1)
        return u_a, h_a
