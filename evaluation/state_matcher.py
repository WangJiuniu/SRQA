...
class StateMatcher(object):
    ...
    def __init__(self):
        ...
        self.PATTERN_ETHNIC_GROUPS = re.compile('^(.+)(%s)$' % regex)
        ...
        regex = build_greedy_regex_str(list(self.state_ids.keys()))
        self.PATTERN_STATE = re.compile(regex)

        names = list(self.state_ids.keys()) + list(self.full_name_ids.keys())
        regex = build_greedy_regex_str(names)
        self.PATTERN_COMPLEX_STATE = re.compile('^(%s)+$' % regex)

    def get_simple_state_name(self, name):
        '''name: a single state name'''
        suffix = name[-1].strip()
        if suffix in '省市县旗区盟州':
            name = name[0:-1].strip()
        if name.endswith('自治'):
            name = name[0:-2].strip()
        if name.endswith('特别行政'):
            name = name[0:-4].strip()
        m = self.PATTERN_ETHNIC_GROUPS.match(name)
        if m: name = m.group(1)
        suffixes = ['维吾尔', '哈萨克', '蒙古', '柯尔克孜', 
                    '塔吉克', '锡伯',]
    ...
    def is_same_complex_state(self, std_text, other_text):
        ...
        if DEBUG: print('std_text:', std_text.encode('utf-8'), file=sys.stderr)
        if DEBUG: print('other_text:', other_text.encode('utf-8'), file=sys.stderr)
        ...
        for sub_names in [std_sub_names, other_sub_names]:
            if DEBUG: print('check is state name: ', end=' ', file=sys.stderr)
            if DEBUG: print(' '.join(sub_names).encode('utf-8'), file=sys.stderr)
            for sub_name in sub_names:
                if not self.is_state_name(sub_name):
                    return False
                if DEBUG: print('\t', sub_name, '\tpass', file=sys.stderr)
            if DEBUG: print('\tevery part is a state name', file=sys.stderr)
        ...
        # verify order
        if DEBUG: print('verify order:', file=sys.stderr)
        if DEBUG: print(' '.join(sub_names).encode('utf-8'), file=sys.stderr)
        for i in range(1, len(sub_names)):
            if not self.is_in(sub_names[i], sub_names[0]):
                return False
            if DEBUG: print('\t', sub_names[i], '|', sub_names[0], '\tpass', file=sys.stderr)
        if DEBUG: print('\torder ok', file=sys.stderr)
