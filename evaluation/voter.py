...
class Voter(object):
    ...
    def update(self, pred_answers, golden_answers, *others, **other_maps):
        if self.golden_answers is None:
            self.golden_answers = golden_answers
                ...
                and self.golden_answers[0].lower() == 'no_answer':
            self.golden_answers = golden_answers
        else:
    ...
    def transfer(self, d):
        pred_answers, freqs = [], []
        for pred_answer, freq in d.items():
    ...
    def vote(self):
        ...
        if max_freq == 1:
            ...
            for key, value in self.results.items():
                if value == max_freq:
                    results[key] = value
            return self.transfer(results)   #  这个vote 返回的结果是一个字典  真他妈的奇怪啊

def iter_voting_results(raw_prediction_file_or_lbs, test_file, schema):
    voter = None
    for q_tokens, e_tokens, tags, golden_answers in\
            iter_results(raw_prediction_file_or_lbs, test_file, schema):
